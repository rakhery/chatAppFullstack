...
type RestOf<T extends IMessage | NewMessage> = Omit<T, 'createdAt' | 'updateAt'> & {
...
export class MessageService {
  ...
  search(req: SearchWithPagination): Observable<EntityArrayResponseType> {
    const options = createRequestOption(req);
    return this.http
      .get<RestMessage[]>(this.resourceSearchUrl, { params: options, observe: 'response' })
      .pipe(map(res => this.convertResponseArrayFromServer(res)));
  }
  ...
  protected convertDateFromClient<T extends IMessage | NewMessage | PartialUpdateMessage>(message: T): RestOf<T> {
    return {
      ...message,
      createdAt: message.createdAt?.toJSON() ?? null,
      updateAt: message.updateAt?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restMessage: RestMessage): IMessage {
    return {
      ...restMessage,
      createdAt: restMessage.createdAt ? dayjs(restMessage.createdAt) : undefined,
      updateAt: restMessage.updateAt ? dayjs(restMessage.updateAt) : undefined,
    };
  }
